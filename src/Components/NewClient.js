    ...
    const createClick = () => {
        if (props.target.firstName === "") {
            ...
            .then(response => {
                ...
                // Create an object to contain the new client data for transferring to the new event module
                let newClient = {
                    ID: data.clientID,
                    firstName: data.firstName,
                    lastName: data.lastName
                };
                props.handleCreateModalOpen(newClient);
                alert(response.data);
            })
            .catch(error => {
                // Failed
        ...
        <Modal show={props.createModalOpen} onHide={props.handleCreateModalOpen}>
            ...
            <Form>
                  <Form.Group className="mb-3" controlId="formClientID">
                      ...
                      <Form.Control name="clientID" type="string" placeholder="Client ID" value={data.clientID} onChange={updateData}/>
                  </Form.Group>
